...
if($_SESSION['sc']!=1)
{
	echo "<script>location='sc.php'</script>";
      							...
      							<?					               		
											if(isset($_POST['submit']))
											{
												...
												$j =1;
 												...
 												$home=mysql_query("SELECT * FROM `home_f_sc` WHERE  `college_name`='$college_name' AND `branch`= '$branch[$i]'  AND `round_2m_s` <= '$mark'");
												...
												$state=mysql_query("SELECT * FROM `state_f_sc` WHERE `college_name`='$college_name' AND `branch`='$branch[$i]' AND `round_2m_s` <= '$mark'");
