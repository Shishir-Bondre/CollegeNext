...
if($_SESSION['obc']!=1)
{
	echo "<script>location='obc.php'</script>";
      							...
      							<?					               		
											if(isset($_POST['submit']))
											{
												...
												$j =1;
 												...
 												$home=mysql_query("SELECT * FROM `home_m_obc` WHERE  `college_name`='$college_name' AND `branch`= '$branch[$i]'  AND `round_1m_s` <= '$mark'");
												...
												$state=mysql_query("SELECT * FROM `state_m_obc` WHERE `college_name`='$college_name' AND `branch`='$branch[$i]' AND `round_1m_s` <= '$mark'");
